...
class LLMService:
    def __init__(self):
        ...
        # 兼容旧代码
        self.api_key = config.get("LLM_API_KEY") or config.get("LLM_API_KEYS", [""])[0]
...
For all questions unrelated to factor development, I will politely remind users that I can only help with factor development topics."""
    ...
    async def chat_completion(self, messages) -> str:
        """发送聊天请求到 OpenAI API（使用多密钥管理器）"""
        try:
            # 格式化消息
            formatted_messages = self._prepare_messages(messages)
            
            # 使用多密钥管理器进行调用（自动轮询和故障转移）
            ...
            logger.info(f"LLM调用成功，使用模型: {self.model}, Token使用: {response_dict['usage']['total_tokens']}")
